...
function parseHighwayRef(ref) {
  if (!ref) return null;
  const m = ref.match(/^(I|US|MD)-(\d+)$/);
  if (!m) return null;
  return { type: m[1], number: parseInt(m[2], 10), isEven: parseInt(m[2], 10) % 2 === 0 };
}

// When multiple routes are concurrent (e.g. I-70 running with US-40),
// MDOT SHA's mile markers and exit numbers follow the Interstate designation,
// not whichever ref happened to sort first out of Overpass. Used for
// milepost lookups and the shield — NOT for the on-screen "I-70 / US-40"
...
// genuine long Interstate-Interstate multiplex (e.g. I-70/I-270, both
// type "I", tied — both should be treated as equally "primary") from a
// short accidental overlap with a lower-tier route. primaryHighwayRef()
// would arbitrarily tie-break two Interstates by array order, which is
// exactly wrong for deciding "should this ref get the full search
// radius" — two same-type refs should both qualify, not just whichever
// sorted first.
...
function shieldCandidates(parsed) {
  ...
  if (parsed.type === 'I') return [`I-${n}.svg`, `Interstate ${n}.svg`, `I-${n} (MD).svg`];
  if (parsed.type === 'US') return [`US ${n}.svg`, `US Highway ${n}.svg`, `US ${n} (MD).svg`];
  if (parsed.type === 'MD') return [`MD Route ${n}.svg`, `Maryland ${n}.svg`, `MD ${n}.svg`];
  return [];
}
...
function mdRouteMatches(attrs, parsedRef) {
  ...
  return type === parsedRef.type && Number(attrs.ID_RTE_NO) === parsedRef.number;
}
...
async function updateMilepostAndDirection(lat, lon) {
  ...
  const withMeta = features.map(f => {
    ...
    const dist = (geom.y != null && geom.x != null)
      ...
      : Infinity;
    return { mp, dist, lat: geom.y, lon: geom.x, attrs: a, direction: null }; // MD's layer has no direction field at all
  });
